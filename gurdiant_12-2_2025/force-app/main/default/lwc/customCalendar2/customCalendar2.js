...
export default class CustomCalendar extends LightningElement {
    ...
    handleAffiliatedGroupChange(event) {
        ...
        const filteredAppointments = this.appointments.filter(
            (appointment) => appointment.extendedProps.appointmentDetail.Provider_Company__c === this.selectedAffiliatedgroup
        );
    ...
    handleGroupLocationChange(event) {
        ...
        const isAffiliatedGroupProvided =
            this.selectedAffiliatedgroup && this.selectedAffiliatedgroup !== 'No Affiliated Group Provided';

        // Filter appointments based on group location and affiliated group if applicable
        const filteredAppointments = this.appointments.filter((appointment) => {
            const { Provider_Company__c, Clinic_Location__c } = appointment.extendedProps.appointmentDetail;

            if (isAffiliatedGroupProvided) {
                // Filter with both affiliated group and group location
                return (
                    Provider_Company__c === this.selectedAffiliatedgroup &&
                    Clinic_Location__c === this.selectedGroupLocation
                );
            } else {
                // Filter only by group location
    ...
    handleProviderTypeChange(event) {
        ...
        const filteredAppointments = this.appointments.filter((appointment) => {
            ...
            if (isAffiliatedGroupProvided && isGroupLocationProvided) {
            ...
            } else if (!isAffiliatedGroupProvided && isGroupLocationProvided) {
    ...
    handleProviderChange(event) {
        ...
        const filteredAppointments = this.appointments.filter(
            (appointment) =>
                appointment.extendedProps.appointmentDetail.Clinic_Resource__c === this.selectedProvider
        );
        this.loadCalendar(
        ...
        await retrievePatientAppointments({ rand: Math.random() }).then(async res => {
            ...
            try {
                ...
                for (const element of JSON.parse(res)) {
                    ...
                    eventTitle = element.Patient_Account__r?.Name || "N/A"; // Safe access with fallback
